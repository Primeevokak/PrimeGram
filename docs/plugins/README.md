...
**Чего у нас нет.** Подмены методов клиента — `hook_method`, `hook_all_methods`,
`hook_all_constructors`. Они требуют Xposed-подобного вмешательства в байткод, которого здесь нет.
Вызовы не падают, а пишут строчку в лог, чтобы плагин, рассчитанный на них, не разваливался
целиком. Файловых и intent-хуков тоже нет.
