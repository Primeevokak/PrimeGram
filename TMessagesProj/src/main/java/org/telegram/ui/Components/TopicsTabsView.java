...
public class TopicsTabsView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate, FactorAnimator.Target {
    ...
    public TopicsTabsView(Context context, BaseFragment fragment, int currentAccount, long dialogId, Theme.ResourcesProvider resourcesProvider) {
        ...
        final int m = org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : 7;
        ...
        topTabs = new UniversalRecyclerView(context, currentAccount, 0, this::fillHorizontalTabs, this::onTabClick, this::onTabLongClick, resourcesProvider) {
            ...
            protected void dispatchDraw(Canvas canvas) {
                ...
                final boolean needClip = (clipAlphaL > 0 || clipAlphaR > 0) && !org.telegram.messenger.NonIslandHelper.chatElements();
                if (needClip) {
    ...
    protected void dispatchDraw(@NonNull Canvas canvas) {
        if (sideTabsContainer.getVisibility() == VISIBLE) {
            sideMenuBackgroundDrawable.setBounds(
                    ...
                    (int) (sideTabsContainer.getTranslationX() + dp(org.telegram.messenger.NonIslandHelper.chatElements() ? SIDE_TABS_WIDTH : (7 + 7 + 64))),
        ...
        if (topTabsContainer.getVisibility() == VISIBLE) {
            ...
            topMenuBackgroundDrawable.setBounds(
                    ...
                    getMeasuredWidth(), (int) (topTabsContainer.getTranslationY() + dp(org.telegram.messenger.NonIslandHelper.chatElements() ? TOP_TABS_HEIGHT : (TOP_TABS_HEIGHT + 7 + 7))));
    ...
    public void setSideMenuBackgroundDrawable(BlurredBackgroundDrawable sideMenuBackgroundDrawable) {
        ...
        this.sideMenuBackgroundDrawable.setRadius(org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : dp(16));
        this.sideMenuBackgroundDrawable.setPadding(org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : dp(7));
    }
    ...
    public void setTopMenuBackgroundDrawable(BlurredBackgroundDrawable sideMenuBackgroundDrawable) {
        ...
        this.topMenuBackgroundDrawable.setRadius(org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : dp(18));
        this.topMenuBackgroundDrawable.setPadding(org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : dp(7));
    }
    ...
    private void checkUi_topicsVerticalPosition() {
        ...
        if (topicBottom) {
            topTabsContainer.setTranslationY(getMeasuredHeight() - dp(TOP_TABS_HEIGHT + (org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : (7 + 7))) - sideMenuBackgroundMarginBottom + lerp(dp(TOP_TABS_HEIGHT + (org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : 7)), 0, getTabsVisibility(Position.BOTTOM)));
        } else {
            topTabsContainer.setTranslationY(sideMenuBackgroundMarginTop + lerp(-dp(TOP_TABS_HEIGHT + (org.telegram.messenger.NonIslandHelper.chatElements() ? 0 : 7)), 0, getTabsVisibility(Position.TOP)));
        }
    }
