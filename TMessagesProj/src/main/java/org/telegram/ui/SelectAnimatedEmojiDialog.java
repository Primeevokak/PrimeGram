        ...
        RecyclerListView.OnItemLongClickListenerExtended onItemLongClick = new RecyclerListView.OnItemLongClickListenerExtended() {
            ...
            public boolean onItemClick(View view, int position, float x, float y) {
                ...
                if (view instanceof ImageViewEmoji && (type == TYPE_REACTIONS || type == TYPE_EXPANDABLE_REACTIONS)) {
                    ...
                    if (!imageViewEmoji.isDefaultReaction && !UserConfig.getInstance(currentAccount).hasRealPremium()) {
    ...
    private class SearchAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            if (holder.getItemViewType() == VIEW_TYPE_HEADER) {
                ...
                if (!UserConfig.getInstance(currentAccount).hasRealPremium() && type == TYPE_EFFECTS && currentReaction.isEffect && currentReaction.premium) {
                ...
                if (searchResult != null && position >= 0 && position < searchResult.size()) {
                    ...
                    if (type == TYPE_EFFECTS && visibleReaction != null && !UserConfig.getInstance(currentAccount).hasRealPremium() && visibleReaction.isEffect && visibleReaction.premium) {
                        imageView.createPremiumLockView();
                        imageView.premiumLockIconView.setVisibility(View.VISIBLE);
                    } else {
                        if (imageView.premiumLockIconView != null) {
            ...
            if (viewType == VIEW_TYPE_HINT) {
                ...
                if (index >= 0) {
                    ...
                    if (type != TYPE_SET_REPLY_ICON && type != TYPE_SET_REPLY_ICON_BOTTOM && type != TYPE_CHAT_REACTIONS) {
                        lock = !pack.free && !UserConfig.getInstance(currentAccount).hasRealPremium();
                    }
                ...
                if (packIndex >= 0 && packIndex < packs.size()) {
                    ...
                    if (pack != null) {
                        button.set(pack.set.title, !pack.free && !UserConfig.getInstance(currentAccount).hasRealPremium(), pack.installed, e -> {
                            if (!pack.free && !UserConfig.getInstance(currentAccount).hasRealPremium()) {
