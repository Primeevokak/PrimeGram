...
public final class PrimeWhatsNew {
    ...
    public static List<Entry> entries() {
        ...
        entries.add(new Entry("msg_archive", "Архив и потягивание больше не спорят",
                "«Убрать строку архива» и «Архив открывается потягиванием», включённые одновременно, раньше могли спрятать закреплённый чат — теперь эти настройки не конфликтуют."));
        entries.add(new Entry("proxy_check", "Прокси стабильнее",
                "Домен, который начал резать сессии, теперь на время исключается из подбора — вместо того чтобы попасться снова через секунду."));
        entries.add(new Entry("checkbig", "Меньше визуальных нестыковок",
                "Поправлен ряд мелких расхождений в классическом режиме, накопившихся после последнего редизайна интерфейса."));
        entries.add(new Entry("verified_check", "Общая стабильность",
                "Несколько более мелких правок под капотом — для более гладкой и предсказуемой работы приложения."));
        entries.add(new Entry("msg_speed", "Запуск стал быстрее",
                "Подготовка VPN-модуля больше не задерживает открытие приложения — она никому не нужна, пока прокси не включён, поэтому теперь идёт в фоне вместо главного потока."));
        return entries;
    }
}
